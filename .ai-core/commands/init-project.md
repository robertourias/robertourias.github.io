...
Você é o agente de inicialização deste scaffold. Seu papel é preencher os arquivos de contexto do `.ai-core/` com as informações reais do produto, eliminando todos os `<!-- TODO -->` e transformando os templates em documentação útil para os agentes que trabalharão neste projeto.
...
### Bloco 1 — Produto (preenche `context/product.md`)
...
Após coletar todas as respostas do Bloco 1, preencha `.ai-core/context/product.md` com o conteúdo real. Remova todos os `<!-- TODO -->`, remova o aviso `**Status do arquivo:** vazio` e substitua pelos dados reais. Informe o caminho do arquivo preenchido antes de continuar.

---

### Bloco 2 — Arquitetura e Stack (preenche `context/architecture.md`)
...
Após coletar todas as respostas do Bloco 2, preencha `.ai-core/context/architecture.md`. Preencha a tabela de tecnologias com as escolhas reais, remova todos os `<!-- TODO -->` e o aviso de status. Informe o caminho do arquivo preenchido antes de continuar.

---

### Bloco 3 — Decisões de Backend (preenche `decisions/backend.md`)

As respostas do Bloco 2 já cobrem ORM, auth, filas e cache. Use-as para preencher `.ai-core/decisions/backend.md` sem fazer novas perguntas — apenas confirme se há algo adicional:
...
Preencha `.ai-core/decisions/backend.md` com as escolhas coletadas. Remova TODOs e aviso de status. Informe o caminho antes de continuar.
...
### Bloco 5 — Glossário (preenche `GLOSSARY.md`)

Use os termos de domínio coletados no Bloco 1 (pergunta 6). Se houver termos, preencha `.ai-core/GLOSSARY.md` com cada termo e sua definição precisa. Se não houver termos definidos ainda, deixe o arquivo com um placeholder comentado e informe o usuário.
...
✅ Arquivos preenchidos:
  ...
  - .ai-core/GLOSSARY.md  (ou: ⚠️ aguardando termos de domínio)
