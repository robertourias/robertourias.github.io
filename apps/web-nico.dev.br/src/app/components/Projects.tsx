...
export default function Projects() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setCardsToShow(w < 768 ? 1 : w < 1024 ? 2 : 3)
    }
    update()
  ...
  // Resetar índice ao mudar breakpoint para evitar index inválido
  useEffect(() => {
    setCurrentIndex(0)
  }, [cardsToShow])

  // ── Desktop nav ───────────────────────────────────────────────
  const totalDesktopPages = Math.ceil(projects.length / cardsToShow)
  const desktopPage = Math.floor(currentIndex / cardsToShow)
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        {/* Header com botões de navegação (desktop) */}
        <div className="mb-12 md:mb-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="hidden md:flex items-center gap-4 mb-4">
            ...
            <button
              onClick={handlePrev}
              className="p-2.5 rounded-full bg-surface-container hover:bg-surface-container-high transition-colors"
        ...
        {isMobile ? (
          ...
          >
            ...
            >
              {projects.map((project: Project) => (
                <article
                  key={project.id}
                  className="shrink-0 w-full group bg-surface-container rounded-xl overflow-hidden"
                  draggable={false}
                >
          ...
          /* ── Desktop: grid com slice de cards visíveis ── */
